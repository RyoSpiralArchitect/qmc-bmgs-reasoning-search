...
def _validate(temporary_root: Path) -> None:
    ...
    run([sys.executable, "-P", "-B", "scripts/check_feedback_budget_provenance.py"])
    ...
    run(
        [
            ...
            "-P",
            "-B",
            str(ROOT / "scripts/feedback_budget_development_admission.py"),
            "--self-test",
        ],
        cwd=outside,
    )
    run(
        [
            ...
            "-P",
            "-B",
            str(ROOT / "scripts/run_feedback_budget_execution.py"),
            "--self-test",
        ],
        cwd=outside,
    )
