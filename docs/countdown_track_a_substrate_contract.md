...
materialized only after an accepted selection charge and only for the selected
source. Its dimension is exactly the legal-action count returned by the task
adapter. The complete ordered action list and its digest are persisted. A
...
transform, point vectors, charge deltas, event chain, and final ledger. It does
...
- a rejected charge consumes no node, point, event, or node-local visit;
